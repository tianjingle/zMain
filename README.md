...
# 2021-7-25修改
- 系统采用了前复权的模式，采用了最简单的baostock的前复权数据，因此决定以后均采用前复权作为系统运算的数据，因此这里删除了老的数据表。重新获取了相关数据。
### 注意点：
1. 为了让三张基础表依然保留，因此我们可以先导出stock_basic、candidate_stock、ajgdy这三种表的表结构和数据，然后将整个数据库删除。
2. 删除数据库之后，我们将刚才导出的表再导入进来
3. 重启zMain获取k线数据。新的数据我们采用15年1月1日开始到当天。

### 潜在的问题和未来的解决方案
1. 如果本地库为复权之后的数据，如果baostock在以后的某一天进行复权，新的复权导致baostock的历史数据有了新的值而本地库尚且是旧的复权数据，可能导致运算结果不准确的问题。
2. 本地库为复权的数据，那么如果系统将股票买入记录到股票池数据库中，相关的收益计算均需要进行复权，否则统计结果不准。
- 解决方案：对于本地股票池目前就不做处理了，如果已经复权的本地数据检测到baostock库已经发生了新的复权运算，我们就删掉当前的股票数据表，并重新从baostock获取最新的复权数据。对于通达信来说，我们决定将最新的股票数据进行更新入库，个人觉得保证最近200天的复权数据即可。
