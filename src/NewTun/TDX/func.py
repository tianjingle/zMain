...
class func:
    ...
    # debug输出函数
    def user_debug(self,print_str, print_value='', ):
        """第一个参数为变量名称，第二个参数为变量的值"""
        if ucfg.user_config().debug:
            if print_value:
    ...
    def day2csv(self,source_dir, file_name, target_dir):
        ...
        if not os.path.isfile(target_path):
            ...
            self.user_debug('当前指针', target_file.tell())
            target_file.seek(0, 0)  # 文件指针移到文件头
            self.user_debug('移动指针到开头', target_file.seek(0, 0))
            target_file_content = target_file.readlines()  # 逐行读取文件内容
            row_number = len(target_file_content)  # 获得文件行数
            self.user_debug('目标文件行数', row_number)
            self.user_debug('目标文件最后一行的数据', target_file_content[-1])
            target_file.seek(0, 2)  # 文件指针移到文件尾
            self.user_debug('移动指针到末尾', target_file.seek(0, 2))
            if row_number > source_row_number:
                self.user_debug('已是最新数据，跳过for循环')
            else:
                self.user_debug('追加模式，从' + str(row_number + 1) + '行开始')
    ...
    def get_TDX_blockfilecontent(self,filename):
        """
        读取本机通达信板块文件，获取文件内容
        :rtype: object
        ...
        if ucfg.user_config().tdx['tdx_path']:
            filepath = ucfg.user_config().tdx['tdx_path'] + os.sep + 'T0002' + os.sep + 'hq_cache' + os.sep + filename
    ...
    def get_lastest_stocklist(self):
        """
        使用pytdx从网络获取最新券商列表
        :return:DF格式，股票清单
        """
    ...
    def historyfinancialreader(self,filepath):
        """
        读取解析通达信目录的历史财务数据
    ...
    def list_localTDX_cwfile(self,ext_name):
        """
        列出本地已有的专业财务文件。返回文件列表
        :param ext_name: str类型。文件扩展名。返回指定扩展名的文件列表
        :return: list类型。财务专业文件列表
        """
        cw_path = ucfg.user_config().tdx['tdx_path'] + os.sep + "vipdoc" + os.sep + "cw"
    ...
    def readall_local_cwfile(self):
        ...
        for cwfile in result:
            if os.path.getsize(ucfg.user_config().tdx['csv_cw'] + os.sep + cwfile) != 0:
                dict[cwfile[4:-4]] = pd.read_pickle(ucfg.user_config().tdx['csv_cw'] + os.sep + cwfile, compression=None)
        print(f'读取所有财报文件完成 用时{(time.time() - starttime_tick):.2f}秒')
        return dict


    def make_fq(self,code, df_code, df_gbbq, df_cw='', start_date='', end_date='', fqtype='qfq'):
        ...
        if df_cw == '':
            cw_dict = self.readall_local_cwfile()
        else:
            cw_dict = df_cw
    ...
    def get_tdx_lastestquote(self,stocklist=None):
        """
        ...
        if stocklist is None:  # 如果列表为空，则获取csv_lday目录全部股票
            stocklist = []
            for i in os.listdir(ucfg.user_config().tdx['csv_lday']):
        ...
        if api.connect(ucfg.user_config().tdx['pytdx_ip'], ucfg.user_config().tdx['pytdx_port']):
    ...
    def update_stockquote(self,code, df_history, df_today):
        """
