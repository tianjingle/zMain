...
# 目录需要事先手动建立好，不然程序会出错
tdx = {
    'tdx_path': 'F:\\Program Files\\Tongdax',  # 指定通达信目录
    'csv_lday': 'F:\\Program Files\\Tongdax\\lday_qfq',  # 指定csv格式日线数据保存目录
    'pickle': 'F:\\Program Files\\Tongdax\\pickle',  # 指定pickle格式日线数据保存目录
    'csv_index': 'F:\\Program Files\\Tongdax\\index',  # 指定指数保存目录
    'csv_cw': 'F:\\Program Files\\Tongdax\\vipdoc\\cw',  # 指定专业财务保存目录
    'csv_gbbq': 'F:\\Program Files\\Tongdax\\',  # 指定股本变迁保存目录
