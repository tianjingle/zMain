...
from src.NewTun.TDX import func
...
class  readTDX_cw:
    ...
    def parseCW(self):
        ...
        tdx_txt_df = func.func().dowload_url(tdx_txt_url)  # 下载gpcw.txt
        ...
        # 检查本机通达信dat文件是否有缺失
        local_zipfile_list = func.func().list_localTDX_cwfile('zip')  # 获取本机已有文件
        many_thread_download = func.func().ManyThreadDownload()
        for df_filename in tdx_txt_df['filename'].tolist():
            ...
            if df_filename not in local_zipfile_list:
                ...
                df = func.func().historyfinancialreader(local_datfile_path)
        ...
        local_datfile_list = func.func().list_localTDX_cwfile('dat')  # 获取本机已有文件
        for filename in local_datfile_list:
            ...
            if filenamepkl not in cwfile_list:  # 本机zip文件的md5与服务器端不一致
                ...
                df = func.func().historyfinancialreader(datpath)
        ...
        filepath = ucfg.user_config().tdx['tdx_path'] + '/T0002/hq_cache/gbbq'
