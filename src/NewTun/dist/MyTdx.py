...
import MyUser_config as ucfg
from src.NewTun.dist import MyFunc
...
class MyTdx:
    ...
    def shouldDeleteTable(self):
        list=[]
        str_p = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
        dateTime_p = datetime.datetime.strptime(str_p, '%Y-%m-%d %H:%M:%S')
        now = (dateTime_p + datetime.timedelta(days=0)).strftime("%Y%m%d")
        yestoday = (dateTime_p + datetime.timedelta(days=-3)).strftime("%Y%m%d")
        print(now)
        print(yestoday)
        path=self.u.tdx['tdx_path']+os.sep+'gbbq.csv'
        ...
        for line in fr.readlines():
            ...
            if len(kkk)>2:
                code=kkk[0]
                date=kkk[1]
                if date==yestoday or date==now:
                    list.append(code+"_"+date)
        fr.close()
        print(list)
    ...
    def parseCW(self):
        ...
        tdx_txt_df = MyFunc.MyFunc().dowload_url(tdx_txt_url)  # 下载gpcw.txt
        ...
        # 检查本机通达信dat文件是否有缺失
        local_zipfile_list = MyFunc.MyFunc().list_localTDX_cwfile('zip')  # 获取本机已有文件
        many_thread_download = MyFunc.MyFunc().ManyThreadDownload()
        for df_filename in tdx_txt_df['filename'].tolist():
            ...
            if df_filename not in local_zipfile_list:
                ...
                local_zipfile_path = ucfg.user_config().tdx['tdx_path'] + os.sep + "vipdoc" + os.sep + "cw" + os.sep + df_filename
                ...
                with zipfile.ZipFile(local_zipfile_path, 'r') as zipobj:  # 打开zip对象，释放zip文件。会自动覆盖原文件。
                    zipobj.extractall(ucfg.user_config().tdx['tdx_path'] + os.sep + "vipdoc" + os.sep + "cw")
                local_datfile_path = local_zipfile_path[:-4] + ".dat"
                df = MyFunc.MyFunc().historyfinancialreader(local_datfile_path)
                csvpath = ucfg.user_config().tdx['csv_cw'] + os.sep + df_filename[:-4] + ".pkl"
        ...
        for zipfile_filename in local_zipfile_list:
            starttime_tick = time.time()
            local_zipfile_path = ucfg.user_config().tdx['tdx_path'] + os.sep + "vipdoc" + os.sep + "cw" + os.sep + zipfile_filename
            ...
            if file_md5 not in tdx_txt_df['md5'].tolist():  # 本机zip文件的md5与服务器端不一致
                ...
                local_datfile_path = local_zipfile_path[:-4] + ".dat"
                df = MyFunc.MyFunc().historyfinancialreader(local_datfile_path)
                csvpath = ucfg.user_config().tdx['csv_cw'] + os.sep + zipfile_filename[:-4] + ".pkl"
                df.to_pickle(csvpath, compression=None)
                print(f'{zipfile_filename} 完成更新 用时 {(time.time() - starttime_tick):>5.2f} 秒')

        # 检查本机财报导出文件是否存在
        cwfile_list = os.listdir(ucfg.user_config().tdx['csv_cw'])  # cw目录 生成文件名列表
        local_datfile_list = MyFunc.MyFunc().list_localTDX_cwfile('dat')  # 获取本机已有文件
        for filename in local_datfile_list:
            starttime_tick = time.time()
            filenamepkl = filename[:-4] + '.pkl'
            pklpath = ucfg.user_config().tdx['csv_cw'] + os.sep + filenamepkl
            filenamedat = filename[:-4] + '.dat'
            datpath = ucfg.user_config().tdx['tdx_path'] + os.sep + "vipdoc" + os.sep + "cw" + os.sep + filenamedat
            if filenamepkl not in cwfile_list:  # 本机zip文件的md5与服务器端不一致
                print(f'{filename} 本机没有 需要导出')
                df = MyFunc.MyFunc().historyfinancialreader(datpath)
        ...
        filepath = ucfg.user_config().tdx['tdx'] + '/T0002/hq_cache/gbbq'
        ...
        df_gbbq.to_csv(ucfg.user_config().tdx['csv_gbbq'] + os.sep + 'gbbq.csv', encoding='gbk', index=False)
