...
def LN(S1):  return np.log(S1)               #返回自然对数
...
def SMA(S, N, M):   #中国式的SMA,至少需要120周期才精确
    com=(N/M)-1 
    return pd.Series(S).ewm(com=com, adjust=False).mean().values
...
def BARSLAST(S_BOOL):                  #上一次条件成立到当前的周期  
    ...
    return len(S_BOOL)-int(M[-1])  if M.size>0 else -1

def barslast(statement):
    series = pd.Series(statement)
    size = len(series)
    end = size
    begin = size - 1
